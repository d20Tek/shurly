...
[TestClass]
public class PublishShortenedUrlEndpointTests
{
    [TestMethod]
    public async Task HandleAsync_WithValidRequest_ReturnsShortenedUrl()
    {
        ...
        // act
        var response = await client.PutAsync($"/api/v1/short-url/{urlId}/publish", null);

        // assert
        response.Should().NotBeNull();
        response.IsSuccessStatusCode.Should().BeTrue();
        await response.ShouldBePublishedState(urlId.ToString(), 1);
    }
    ...
    [TestMethod]
    public void PublishShortenedUrlRequest_Setters()
    {
        // arrange
        var id = Guid.NewGuid();
        ...
        var request = new PublishShortenedUrlRequest(Guid.Empty, null!, null!);
